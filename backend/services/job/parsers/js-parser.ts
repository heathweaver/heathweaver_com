...
export class JavaScriptParser extends BaseParser {
  ...
  private extractFromScripts(doc: Document, rawHtml: string): string | null {
    ...
    for (const script of scripts) {
      ...
      console.debug(
        "JavaScript parser: Checking script content:",
      ...
      for (const pattern of patterns) {
        ...
        if (match && match[1]) {
          try {
            ...
            console.debug(
              "JavaScript parser: Found match, trying to parse JSON",
            );
            ...
            console.debug(
              "JavaScript parser: Failed to parse script JSON:",
              err,
            );
            continue;
          }
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1]) {
        try {
          ...
          console.debug(
            "JavaScript parser: Found match in raw HTML, trying to parse JSON",
          );
          const data = JSON.parse(jsonStr);
          if (this.isJobData(data)) {
            console.debug(
              "JavaScript parser: Found valid job data in raw HTML",
            );
            return this.extractJobContent(data);
          }
        } catch (err) {
          console.debug(
            "JavaScript parser: Failed to parse raw HTML JSON:",
            err,
          );
          continue;
        }
  ...
  private unescapeZohoJson(escaped: string): string {
    ...
    console.debug(
      "JavaScript parser: Unescaped first 100 chars:",
  ...
  private isJobData(data: unknown): boolean {
    if (Array.isArray(data)) {
      ...
      const hasJobData = data.some((item) =>
        item &&
        typeof item === "object" &&
        ("Job_Description" in item || "jobDescription" in item ||
          "description" in item)
      );
  ...
  private extractJobContent(data: unknown): string {
    if (Array.isArray(data)) {
      ...
      if (item && typeof item === "object") {
        const content = this.cleanHtmlContent(
          item.Job_Description as string || "",
        );
        console.debug(
          "JavaScript parser: Extracted content from array (first 100 chars):",
          content.substring(0, 100),
        );
        return content;
      }
    ...
    console.debug(
      "JavaScript parser: Extracted content from object (first 100 chars):",
      content.substring(0, 100),
    );
    return content;
  }
