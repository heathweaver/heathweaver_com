...
export class JobContentValidator implements ContentValidator {
  ...
  validate(content: string): { isValid: boolean; reason?: string; score?: number } {
    if (!content || content.length < this.config.minContentLength) {
      return {
        isValid: false,
        reason: `Content length (${content?.length || 0}) below minimum (${this.config.minContentLength})`,
        score: 0
      };
    }
    ...
    if (foundTerms.length < this.config.minJobTerms) {
      return {
        isValid: false,
        reason: `Found only ${foundTerms.length} job terms (minimum ${this.config.minJobTerms})`,
        score
      };
    }
