  ...
  async GET(ctx) {
    ...
    try {
      ...
      console.log("Request details:", {
        ...
        stateFromCookie: req.headers.get("cookie")?.match(
          /oauth-session=([^;]+)/,
        )?.[1],
        kvKey: `oauth-${
          req.headers.get("cookie")?.match(/oauth-session=([^;]+)/)?.[1]
        }`,
      });
      const { response, sessionId, tokens } = await handleTwitterSignupCallback(
        req,
      );
