...
const services: Record<
  string,
  AnthropicService | XAIService | DeepSeekService
> = {
...
export async function handler(
  ...
  try {
    const { message, conversationHistory, model = "anthropic" } = await req
      .json();
    ...
    if (urlPattern.test(message)) {
      ...
      if (!jobResult.success || !jobResult.content) {
        ...
        return new Response(
          JSON.stringify({
            error: jobResult.error || "No content found in job posting",
          }),
          { status: 400, headers: { "Content-Type": "application/json" } },
        );
      }

      const response = await chatHandler.handleJobUrl(
        message,
        model as ChatModel,
        jobResult.content,
      );
